                     ...
                     L10N MLP (Multi-Lingual Publishing) Pack.
                               Copyright  2007-2008
                        Graeme Porteous & Steve (Net-Carver)
...
MLP
 ...
 |
 |-- optional (TxP404 only. You don't have to install these but they help)
 |        |
 |        |-- textpattern
 |                    |-- publish.php
 |
...
you don't need to do this as this was fixed in that revision. 

However, if your version is pre-2146 and you make use of the native txp 
previous and next tags then you might want to consider using the copy of 
publish.php supplied in the /optional/textpattern folder as this fixes the
getNeighbour() routine.


3) (MANDATORY) If upgrading from the gbp_l10n plugin disable the gbp_l10n plugin
now. 


4) (MANDATORY) Install and activate the three files from the /plugin(s) 
...
In standard TxP an 'article' represents the totality of an author's ideas on a 
given matter rendered in one language. That's fine and works well when 
...
language simultaneously and the authors/editors or other TxP users will also 
...
No new interface to learn, just the standard write tab.

Once they are happy with the work, they clear the url-only-title and save the 
article setting it's status as appropriate. On small installations where the 
translator is the owner/publisher/editor that might be 'live' but on larger 
systems with editorial control, they might set the status to pending.
